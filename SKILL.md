...
```bash
cd ${CLAUDE_SKILL_DIR}/scripts && npm install --silent 2>/dev/null && node prepare-digest.js 2>/dev/null
```
...
- `podcasts` — 播客节目及完整文字稿
- `cnArticles` — 中文科技媒体文章
- `officialBlogs` — AI 大厂官方博客（OpenAI / Google / Hugging Face）
- `crossSignals` — 交叉信号（多人共同提及的话题）
- `prev` — 昨日快照（用于变化洞察，首日为 null）
- `prompts` — 混编指令
...
**读取混编指令：** 从 skill 目录的 `prompts/` 文件夹直接读取（不在 JSON 里）：
```bash
cat ${CLAUDE_SKILL_DIR}/prompts/digest-intro.md
cat ${CLAUDE_SKILL_DIR}/prompts/summarize-tweets.md
cat ${CLAUDE_SKILL_DIR}/prompts/summarize-cn-articles.md
cat ${CLAUDE_SKILL_DIR}/prompts/summarize-podcast.md
cat ${CLAUDE_SKILL_DIR}/prompts/signal-guide.md
cat ${CLAUDE_SKILL_DIR}/prompts/daily-diff.md
cat ${CLAUDE_SKILL_DIR}/prompts/translate.md
```
可一次性全部读取，节省轮次。
...
每条推文的 `_metrics` 和顶层的 `crossSignals` 提供量化指标辅助你的判断。
